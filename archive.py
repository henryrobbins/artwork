...
for series in json.load(open("series.json"))["series"]:
    ...
    os.makedirs('archive' / Path(series), exist_ok=True)
    ...
    copyfile(lbl_path, 'archive' / lbl_path)
    description_path = Path(series) / 'description.md'
    copyfile(description_path, 'archive' / description_path)
    ...
    for work in lbl_json['works']:
        ...
        if file_extension == '.png':
            copyfile(from_path, 'archive' / to_path)
            ...
            to_path = 'archive' / to_path.with_suffix('.png')
