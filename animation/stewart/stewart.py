...
def mod(M,k):
    ...
    M = np.array(list(map(lambda x: x % k, M)))*int(256/64)
...
frames = (
    ...
    shift([channel(f,f3(),f3(),f3()) for f in clip("town_tour")], 1) +
    [mod(f,64) for f in clip("pinches", 29, 35)] +
    shift([clip_gradient(f,100,200) for f in clip("henry_movement")], 20) +
    [channel(f,f1(0),f2(16,5),f3()) for f in clip("pinches", 35, 40)] +
    shift([clip_gradient(f,100,200) for f in clip("ella_movement")], 20) +
    shift([channel(f,f3(),f3(),f3()) for f in clip("destroy_town", 0, 19)], 1) +
    [clip_gradient(f,100,200) for f in clip("pinches", 0, 5)] +
    shift([channel(f,f3(),f3(),f3()) for f in clip("destroy_town", 18, 44)[::-1]], -5) +
    [channel(f,lambda x: x,lambda x: 0.5, lambda x: 0.6, 'Lab')
        for f in clip("pinches", 40, 45)] +
    shift([mod(f,64) for f in clip("destroy_town", 18, 44)], 5) +
    [mod(f,32) for f in clip("pinches", 49, 53)] +
    shift(plate_frames[15:29][::-1] +
        plate_frames[15:] +
        plate_frames[24:29][::-1] +
        plate_frames[24:], 8) +
    [clip_gradient(f,0,100) for f in clip("pinches", 22, 28)] +
    shift([channel(f,f3(),f3(),f3()) for f in clip("tree_flowers", 4, 22)], 10) +
    shift([channel(f,f3(),f3(),f3()) for f in clip("tree_flowers", 4, 22)[::-1]], -10) +
    [channel(f,lambda x: x,lambda x: 0.55, lambda x: 0.6, 'Lab')
        for f in clip("pinches", 10, 14)] +
    [np.zeros((189,255,3))]*10
)
